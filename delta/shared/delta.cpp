...
BOOL delta::running(void)
{
	return delta::initialize();
}
...
static BOOL delta::initialize(void)
{
	if (game_handle)
	{
	...
	if (!game_handle)
	{
		LOG("delta process not found\n");
		return 0;
	}
    LOG("delta process found\n");
    return 0;
...
QWORD delta::sdl::get_window(void)
{
	return vm::read_i64(game_handle, sdl::sdl_window);
}

BOOL delta::sdl::get_window_info(QWORD window, WINDOW_INFO *info)
{
	typedef struct 
	{
		int x,y,w,h;
...
QWORD delta::sdl::get_window_data(QWORD window)
{
	return vm::read_i64(game_handle, window + 0x168);
}

QWORD delta::sdl::get_hwnd(QWORD window_data)
{
	return vm::read_i64(game_handle, window_data + 0x08);
}

QWORD delta::sdl::get_hdc(QWORD window_data)
{
	return vm::read_i64(game_handle, window_data + 0x10);
}

QWORD delta::sdl::get_mouse(void)
{
	/*
	    0x60 float x;
	    0x64 float y;
...
BOOL delta::sdl::get_mouse_button(QWORD mouse)
{
	DWORD i;
	DWORD buttonstate = 0;
...
QWORD delta::engine::get_convar(const char *name)
{
...
DWORD delta::engine::get_current_ms(void)
{
...
vec2 delta::engine::get_viewangles(void)
{
	vec2 va{};
...
view_matrix_t delta::engine::get_viewmatrix(void)
{
...
QWORD delta::entity::get_local_player_controller(void)
{
	return vm::read_i64(game_handle, direct::local_player);
}

QWORD delta::entity::get_client_entity(int index)
{
...
BOOL delta::entity::is_player(QWORD controller)
{
	/*
	if (vm::get_target_os() == VmOs::Linux)
	{
...
QWORD delta::entity::get_player(QWORD controller)
{
	//
...
DWORD delta::player::get_health(QWORD player)
{
	return vm::read_i32(game_handle, player + netvars::m_iHealth);
}

DWORD delta::player::get_team_num(QWORD player)
{
	return vm::read_i32(game_handle, player + netvars::m_iTeamNum);
}

int delta::player::get_life_state(QWORD player)
{
	return vm::read_i32(game_handle, player + netvars::m_lifeState);
}

vec3 delta::player::get_origin(QWORD player)
{
	vec3 value{};
...
float delta::player::get_vec_view(QWORD player)
{
	return vm::read_float(game_handle, player + netvars::m_vecViewOffset + 8);
}

vec3 delta::player::get_eye_position(QWORD player)
{
...
DWORD delta::player::get_shots_fired(QWORD player)
{
	return vm::read_i32(game_handle, player + netvars::m_iShotsFired);
}

float delta::player::get_fov_multipler(QWORD player)
{
	return vm::read_float(game_handle, player + netvars::m_flFOVSensitivityAdjust);
}

vec2 delta::player::get_vec_punch(QWORD player)
{
	vec2 data{};
...
vec2 delta::player::get_viewangle(QWORD player)
{
	vec2 value{};
...
delta::WEAPON_CLASS delta::player::get_weapon_class(QWORD player)
{
	QWORD weapon = vm::read_i64(game_handle, player + netvars::m_pClippingWeapon);

	if (weapon == 0)
	{
		return delta::WEAPON_CLASS::Invalid;
	}
	...
	/* knife */
	{
		...
		for (int i = 0; i < sizeof(data) / sizeof(*data); i++)
		{
			...
			{
				return delta::WEAPON_CLASS::Knife;
			}
		}
	}
	...
	/* grenade */
	{
		...
		for (int i = 0; i < sizeof(data) / sizeof(*data); i++)
		{
			if (data[i] == weapon_index)
			{
				return delta::WEAPON_CLASS::Grenade;
			}
		}
	}
	...
	/* pistol */
	{
		...
		for (int i = 0; i < sizeof(data) / sizeof(*data); i++)
		{
			if (data[i] == weapon_index)
			{
				return delta::WEAPON_CLASS::Pistol;
			}
		}
	}
	...
	/* sniper */
	{
		...
		for (int i = 0; i < sizeof(data) / sizeof(*data); i++)
		{
			if (data[i] == weapon_index)
			{
				return delta::WEAPON_CLASS::Sniper;
			}
		}
	}

	return delta::WEAPON_CLASS::Rifle;
}

QWORD delta::player::get_node(QWORD player)
{
	return vm::read_i64(game_handle, player + netvars::m_pGameSceneNode);
}

QWORD delta::player::get_weapon_address(QWORD player)
{
	...
	return delta::entity::get_client_entity(index);
}

BOOL delta::player::is_defusing(QWORD player)
{
	return (BOOL)vm::read_i8(game_handle, player + netvars::m_bIsDefusing);
}

BOOL delta::player::is_valid(QWORD player, QWORD node)
{
	if (player == 0)
	{
		return 0;
...
BOOLEAN delta::node::get_dormant(QWORD node)
{
	return vm::read_i8( game_handle, node + netvars::m_bDormant );
}

vec3 delta::node::get_origin(QWORD node)
{
	vec3 val{};
...
BOOL delta::node::get_bone_position(QWORD node, int index, vec3 *data)
{
	// get skeleton 
...
static QWORD delta::get_interface(QWORD base, PCSTR name)
{
	if (base == 0)
	{
		return 0;
...
QWORD delta::get_interface_function(QWORD interface_address, DWORD index)
{
