...
namespace features
{
	...
	//
	delta::WEAPON_CLASS weapon_class;
...
{
	return min + delta::engine::get_current_ms() % (max + 1 - min);
}

inline void delta::features::update_settings(void)
{
	int crosshair_alpha = delta::get_crosshairalpha();
	...
	switch (weapon_class)
	{
	case delta::WEAPON_CLASS::Knife:
	case delta::WEAPON_CLASS::Grenade:
		config::aimbot_enabled = 0;
		break;
	case delta::WEAPON_CLASS::Pistol:
...
//
static void delta::features::has_target_event(QWORD local_player, QWORD target_player, float fov, vec2 aim_punch, vec3 aimbot_angle, vec2 view_angle, vec3 bone)
{
#ifndef __linux__
	...
	if (b_triggerbot_button && config::aimbot_enabled && mouse_down_ms == 0 && event_state == 0)
	{
		...
		if (weapon_class == delta::WEAPON_CLASS::Pistol)
		{
		...
		if (aim_punch.x > accurate_shots_fl)
		{
			...
			vec3 eye = delta::player::get_eye_position(local_player);
			...
			if (math::vec_min_max(eye, dir,
				...
				coll.radius))
			{
				DWORD current_ms = delta::engine::get_current_ms();
...
void delta::features::run(void)
{
	...
	QWORD local_player_controller = delta::entity::get_local_player_controller();
	...
	QWORD local_player = delta::entity::get_player(local_player_controller);
	...
	weapon_class = delta::player::get_weapon_class(local_player);
	if (weapon_class == delta::WEAPON_CLASS::Invalid)
	{
		return;
	}
	...
	// update buttons
	//
	b_triggerbot_button = delta::input::is_button_down(config::triggerbot_button);
	b_aimbot_button     = delta::input::is_button_down(config::aimbot_button) | b_triggerbot_button;
	...
	BOOL  ffa         = delta::gamemode::is_ffa();
	DWORD num_shots   = delta::player::get_shots_fired(local_player);
	vec2  aim_punch   = delta::player::get_vec_punch(local_player);
	float sensitivity = delta::mouse::get_sensitivity() * delta::player::get_fov_multipler(local_player);
	...
	{
		if (!delta::player::is_valid(aimbot_target, delta::player::get_node(aimbot_target)))
		{
			aimbot_target = best_target;
	...
	QWORD node = delta::player::get_node(aimbot_target);
	if (node == 0)
	{
		return;
	}

	vec2 view_angle = delta::player::get_viewangle(local_player);
	...
	if (config::aimbot_multibone)
	{
		...
		{
			vec3 pos{};
			if (!delta::node::get_bone_position(node, aimbot_bone, &pos))
			{
				return;
			}
			aimbot_pos   = pos;
		...
		else
		{
			...
			{
				vec3 pos{};
				if (!delta::node::get_bone_position(node, i, &pos))
				{
					continue;
				}
	...
	{
		vec3 head{};
		if (!delta::node::get_bone_position(node, 6, &head))
		{
			return;
		}
		aimbot_pos   = head;
...
static vec3 delta::features::get_target_angle(QWORD local_player, vec3 position, DWORD num_shots, vec2 aim_punch)
{
	vec3 eye_position = delta::node::get_origin(delta::player::get_node(local_player));
	eye_position.z    += delta::player::get_vec_view(local_player);

	//
	// which one is better???
	//
	// vec3 eye_position = delta::player::get_eye_position(local_player);
	...
	if (num_shots > 0)
	{
		if (weapon_class == delta::WEAPON_CLASS::Sniper)
			goto skip_recoil;
...
static void delta::features::get_best_target(BOOL ffa, QWORD local_controller, QWORD local_player, DWORD num_shots, vec2 aim_punch, QWORD *target)
{
	vec2 va = delta::player::get_viewangle(local_player);
	...
	for (int i = 1; i < 64; i++)
	{
		QWORD ent = delta::entity::get_client_entity(i);
		...
		// is controller
		//
		if (!delta::entity::is_player(ent))
		{
			continue;
		}

		QWORD player = delta::entity::get_player(ent);
		if (player == 0)
		{
			continue;
		}
		...
		if (ffa == 0)
		{
			if (delta::player::get_team_num(player) == delta::player::get_team_num(local_player))
			{
				continue;
			}
		}

		QWORD node = delta::player::get_node(player);
		if (node == 0)
		{
			continue;
		}

		if (!delta::player::is_valid(player, node))
		{
			continue;
		}
...
static void delta::features::standalone_rcs(DWORD num_shots, vec2 vec_punch, float sensitivity)
{
	if (num_shots > 1)
	{
...
static void delta::features::esp(QWORD local_player, QWORD target_player, vec3 head)
{
	QWORD sdl_window = delta::sdl::get_window();
	if (sdl_window == 0)
		return;


	delta::WINDOW_INFO window{};
	if (!delta::sdl::get_window_info(sdl_window, &window))
		return;

	/*
	float view = delta::player::get_vec_view(local_player) - 10.0f;
	...
	view_matrix_t view_matrix = delta::engine::get_viewmatrix();
	...
	float target_health = ((float)delta::player::get_health(target_player) / 100.0f) * 255.0f;
...
#else
	QWORD sdl_window_data = delta::sdl::get_window_data(sdl_window);
	if (sdl_window_data == 0)
		return;
	QWORD hwnd = delta::sdl::get_hwnd(sdl_window_data);
	...
	*/
	...
	vec3 origin = delta::player::get_origin(target_player);
