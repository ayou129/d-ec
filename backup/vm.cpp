...
static BOOL initialize(void)
{
	if (vmdll == 0)
	{
		...
		auto result = VMMDLL_Initialize(3, args);
		std::cout << "Result of VMMDLL_Initialize: " << result << std::endl;
		vmdll = result; // 000000000CBCD040
	}
...
vm_handle vm::open_process_ex(PCSTR process_name, PCSTR dll_name)
{
	...
	for (i = 0; i < process_count; i++)
	{
		...
		{
			std::string keyword = "64";

			// 先将 LPSTR 转换为 std::string
			std::string moduleName = std::string(module_map->pMap[j].uszText);

			// 判断是否以 .exe 结尾并且包含关键词
			if (moduleName.size() >= 4 &&
				moduleName.substr(moduleName.size() - 4) == ".exe" &&
				moduleName.find(keyword) != std::string::npos) {
